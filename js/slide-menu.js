...
var slideMenu = function(options){
	// Default values
	var kkStart = $("#product-category a.active").attr("data-target");
	var defaults = {
		div: "#menu-container",
	...
	jQuery(o.div + " ul").css({'padding':'0','margin':'0'});
	
	var v = {
		middle: jQuery(o.div).width() / 2,
		ul_w: jQuery(o.div + " ul:eq(" + o.start + ")").outerWidth(),
		active: o.start,
	...
	// by kk - set imgae margin and size
		...
		$(object).each(function(index){
			...
			if(imgRaito >= ratio)
			{
				...
				if(marginH != 0)
					...
					marginH = 0;
				$(this).css({'margin':marginV+'px '+marginH+'px','height':height+'px'});
			...
			else
			{
				...
				marginH = 0;
				$(this).css({'margin-top':marginV+'px','width':width+'px'});
				$(this).animate({'opacity':'1'},300);
			}
		});
	}
	
	function hoverFade(object)
	{
	...
	function try_animate_in(index){
		...
		if( v.loop_started == true && count == 0){
			...
			setImgMargin(o.div+" ul li img",o.frame_width,o.frame_height);
			hoverFade(o.div+" ul li a");
			// Hide loader
			if (o.loader != false && o.loader != '') jQuery(o.loader).hide();
			// Animate in
			jQuery(o.div + " ul:eq(" + o.start + ") li").each(function(index){
				v.x = ((o.x / 2) + ((v.s_l / 2) * o.x) - (o.x * (index + 1))) - v.ul_w;
