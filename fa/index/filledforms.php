...
if($isAdmin)
{
	if($_GET["mode"] == 'search')
	{
		...
		while($ffrows = mysql_fetch_row($query))
		{
			$j++;
		?>
		<div class="ffield_tr <?php if($ffrows[($num_fields+1)] == '0') echo "unseen";?>" <?php if($ffrows[($num_fields+1)] == '0') echo 'title="Ù…Ø´Ø§Ù‡Ø¯Ù‡ Ù†Ø´Ø¯Ù‡"'; ?> id="ff<?php echo $ffrows[0];?>">
			...
			?>
			<div class="member_field <?php if($j%2 == 1) echo "fodd"; else echo "feven";?>" style="width: <?php echo $field_width; ?>%;"  >
				<div style="padding: 0 3px 0 0;margin: 0;float: left;width: 100px;position: relative;margin-top: -4px;" id="fctrl">
					<img src="../images/wrongbw.png" title="حدف" onmouseover="this.src='../images/wrong.png';" onmouseout="this.src='../images/wrongbw.png';" style="float: left;margin: 0 5px 0 0;height: 20px;cursor: pointer;" onclick="delFForm(<?php echo $ffrows[0];?>);">
					<img src="../images/editbw.png" title="اصلاح اطلاعات فرم" onmouseover="this.src='../images/editc.png';" onmouseout="this.src='../images/editbw.png';" style="float: left;margin: 0 5px 0 0;height: 20px;cursor: pointer;" onclick="editFForm(<?php echo $ffrows[0];?>);">
					<a href="index/printfilledform.php?form_id=<?php echo $form_id; ?>&id=<?php echo $ffrows[0]; ?>" class="download" target="_blank"><img src="../images/printbw.png" title="نسخه چاپی" onmouseover="this.src='../images/printc.png';" onmouseout="this.src='../images/printbw.png';" style="float: left;margin: 0 5px 0 0;height: 20px;cursor: pointer;"></a>
				</div>
			</div>
		</div>
		<?php 
	...
	else if($_GET["mode"] == 'change' || $_GET["mode"] == 'applychange')
	{
		...
		$("#page-nav").css({'opacity':'0'}).empty().load('index/pagenavigator.php?page=filledform&form_id=<?php echo $form_id; ?>&id=<?php echo $id; ?>', function() {
			...
			<div style="border: 1px #ccc solid;padding: 0px;margin: 15px auto 0;height: 55px;background-color: #eee;overflow: hidden;border-top-left-radius: 7px;border-top-right-radius: 7px;width: 950px;border-bottom: none;">
				<div style="height: 50px;width: 25%;float: right;padding: 0;margin: 0;">
					<?php 
					if($frow[2] != '')
					{
					?>
					<img alt="<?php echo $frow[1]; ?>" src="../<?php echo $frow[2]; ?>" style="float: right;height: 55px;">
					<?php 
					}
					?>
				</div>
				<div style="height: 40px;width: 30%;float: right;padding: 15px 0 0 0;margin: 0;text-align: center;">
					<span><span style="color: #555;">عنوان:</span> <?php echo $frow[1]; ?></span>
				</div>
				<div style="height: 40px;width: 22.5%;float: right;padding: 15px 0 0 0;margin: 0;text-align: center;">
					<span><span style="color: #555;">کد سند:</span> <?php echo $frow[4]; ?></span>
				</div>
				<div style="height: 40px;width: 22.5%;float: right;padding: 15px 0 0 0;margin: 0;text-align: center;">
					<span><span style="color: #555;">طبقه بندی:</span> <?php echo $frow[5]; ?></span>
				</div>
			</div>
			<form action="index/filledforms.php?mode=applychange" method="POST" enctype="multipart/form-data" id="fillform_form" >
				<div style="border: 1px #ccc solid;padding: 0px;margin: 0 auto 15px;width: 950px;overflow: hidden;border-bottom-left-radius: 7px;border-bottom-right-radius: 7px;position: relative;background-color: #eee;">
					...
					<div <?php if($afs[1] == '4' || ($afs[1] == '3' && $afs[0] == '2' )) echo 'style="height:20px;"'; ?> class="<?php 
						...
						<?php 
							...
							else if($afs[1] == '5')
							{
								?>
									<div class="flable" ><?php if($afs[2] == '1') echo "<span class='red' >*</span>"; ?><?php echo $f_name; ?></div>
									<div style="float: right;margin:4px 0 0 16px;">
								<?php
									$cc = false;
									$other_value = '';
									...
									{
									?>
									<span class="input_span" >دیگر</span>
				...
				?>
					<div class="fbig" style="text-align: right;height: auto;">
						<pre style="font-family: tahoma;font-size: 13px;text-align: right;margin: 0;padding: 0;"><?php echo $frow[151]; ?></pre>
					</div>
				<?php
				}
				?>
	...
	<div id="delete_dialogF" title="حذف فرم" style="display: none;">
	...
	<div style="opacity:0;width: 100%;height: 100%;" id="index-loader">
		<div id="member-container">
			<div class="mheader" >
				...
				<input type="text" id="fsearch" style="float: right;width: 150px;font-size: 10px;" class="textbykk">
			</div>
			<?php 
			if(isset($_GET["form_id"]))
			{
			...
			<div id="ffield_container">
				<?php 
					...
					<div class="ffield_tr <?php if($ffrows[($num_fields+1)] == '0') echo "unseen";?>" <?php if($ffrows[($num_fields+1)] == '0') echo 'title="Ù…Ø´Ø§Ù‡Ø¯Ù‡ Ù†Ø´Ø¯Ù‡"'; ?> id="ff<?php echo $ffrows[0];?>">
						...
						<div class="member_field <?php if($j%2 == 1) echo "fodd"; else echo "feven";?>" style="width: <?php echo $field_width; ?>%;"  >
							<div style="padding: 0 3px 0 0;margin: 0;float: left;width: 100px;position: relative;margin-top: -4px;" id="fctrl">
								<img src="../images/wrongbw.png" title="حذف" onmouseover="this.src='../images/wrong.png';" onmouseout="this.src='../images/wrongbw.png';" style="float: left;margin: 0 5px 0 0;height: 20px;cursor: pointer;" onclick="delFForm(<?php echo $ffrows[0];?>);">
								<img src="../images/editbw.png" title="اصلاح اطلاعات فرم" onmouseover="this.src='../images/editc.png';" onmouseout="this.src='../images/editbw.png';" style="float: left;margin: 0 5px 0 0;height: 20px;cursor: pointer;" onclick="editFForm(<?php echo $ffrows[0];?>);">
								<a href="index/printfilledform.php?form_id=<?php echo $form_id; ?>&id=<?php echo $ffrows[0]; ?>" class="download" target="_blank"><img src="../images/printbw.png" title="نسخه چای" onmouseover="this.src='../images/printc.png';" onmouseout="this.src='../images/printbw.png';" style="float: left;margin: 0 5px 0 0;height: 20px;cursor: pointer;"></a>
							</div>
						</div>
					</div>
					<?php 
