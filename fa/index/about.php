...
if($_GET["mode"] == 'addsection' && $isAdmin)
{
	...
	<div style="opacity:0;width: 100%;height: 100%;" id="index-loader">
		<div class="div-title">افزودن بخش</div>
...
else if(($_GET["mode"] == 'edit' || $_GET["mode"] == 'applyedit' )&& $isAdmin)
{
	...
	{
	?>
		<div class="div-title">تغییر بخش</div>
		<form action="index/about.php?mode=applyedit" method="POST" enctype="multipart/form-data" id="addaboutsection_form" >
			...
			<div class="div-row">
				<div class="input-div" style="width: 80%;" >
					<div class="lable" ><span class="red" >*</span>قابلیت نمایش</div>
					<select class="textbykk" name="showto" id="showto" style="width: 412px;" >
						<option value="1" <?php if($arow[6] == 1) echo "selected";?> >همه</option>
						<option value="2" <?php if($arow[6] == 2) echo "selected";?> >اعضا</option>
						<option value="3" <?php if($arow[6] == 3) echo "selected";?> >اعضای سطح 1</option>
						<option value="4" <?php if($arow[6] == 4) echo "selected";?> >اعضای سطح 2</option>
						<option value="5" <?php if($arow[6] == 5) echo "selected";?> >اعضای سطح 3</option>
						<option value="0" <?php if($arow[6] == 0) echo "selected";?> >فقط مدیر</option>
					</select>
				</div>
			</div>
			<div class="div-row" >
...
else
{
	if($_GET["mode"] == 'addthissection' && $_POST["addsection"] == '1' && $isAdmin)
	{
		...
		else
		{
			...
			<div id="success_dialog" style="display: block;opacity:1;top: 40%;height: 60px;">
			<img alt="correct" src="../images/correct.png" style="float: right;position: relative;height: 60px;"/>
			<div style="font-size: 15px;color: #333;padding: 20px 5px;font-weight: bold;float:right;">بخش اضافه شد!</div>
			<script type="text/javascript">
			$("#success_dialog").click(function(){
					$(this).remove();
				});
...
<?php 
	...
	{
	?>
	<div id="delete_dialogA" title="حذف بخش" style="display: none;">
	<span class="ui-icon ui-icon-alert" style="float:right; margin:0 0px 20px 7px;"></span>مطمئن هستید؟
	</div>
	<?php 
	}
?>
...
$("#controlpanel_container .controls").empty().css({'opacity':'0.0'}).append(
	"<div class='btn add' onclick='addAboutSection();' title='افزودن بخش' ></div><div class='btn change' onclick='editAboutSection(<?php echo $arow[0]; ?>);' title='تغییر این بخش' ></div><div class='btn delete' onclick='deleteAboutSection(<?php echo $nrow[0]; ?>);' title='حذف این بخش' ></div>"
...
<div style="opacity:0;width: 100%;height: 100%;" id="index-loader">
	...
	$(".about-category").mouseenter(function(){
		...
		$(".about-category").not(this).children(".text").animate({'padding-right':'10px'},300);
