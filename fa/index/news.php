...
if($_GET["show"] == '1')
{
	...
	<div class="container" >
		...
		{
		?>
		<div class="news-attachment" ><img src="../images/attachment.png"><a href="<?php echo $nrow[7];?>" class="download" target="_blank">فایل پیوست</a></div><br>
		<?php 
		}
	...
	$("#controlpanel_container .controls").empty().css({'opacity':'0.0'}).append(
			"<div class='btn add' onclick='addNews();' title='افزودن خبر/مقاله' ></div><div class='btn change' onclick='editNews(<?php echo $nrow[0]; ?>);' title='تغییر خبر/مقاله' ></div><div class='btn delete' onclick='deleteNews(<?php echo $nrow[0]; ?>);' title='حذف خبر/مقاله' ></div>"
...
<div style="opacity:0;width: 100%;height: 100%;" id="index-loader">
		...
		?>
			<div class="n-item big" id="_<?php echo $rows[0]; ?>">
				...
				<div class="control-container">
					...
					<div class="visit-counter" >بازدید: <?php echo $rows[8]; ?></div>
				</div>
				<?php 
				}
				?>
				<div class="n-inner" >
					<div style="padding: 3px 5px 5px 5px;position: absolute;top:2px;left: 2px;background-color: #eee;border-bottom-right-radius:5px;text-align: center;font-size: 12px;"><?php echo $rows[1]; ?></div>
					<?php 
					if($rows[4] != '')
					{
					?>
	...
	function SetNewsItemPosition()
	{
		...
		$("#news-items .container .n-item").eq(0).css({'opacity':'1','display':'block','height':'0px','top':(10 + (newsDiv[0][2]/2))+'px','width':'0px','right':(10 + (232/2))+'px'})
			.delay(150).animate({'height':newsDiv[0][2]+'px','top':10+'px','width':464+'px','right':10+'px'},300,'easeOutCirc',function(){
		...
		for(var i = newsToSet; i < newsCount + 1; i++)
		{
			...
			objectDiv.css({'opacity':'1','display':'block','height':'0px','top':(topDiv + (heightDiv/2))+'px','width':'0px','right':(leftDiv + (widthDiv/2))+'px'})
				.delay(i*150).animate({'height':heightDiv+'px','top':topDiv+'px','width':widthDiv+'px','right':leftDiv+'px'},300,'easeOutCirc',function(){
				$(this).children(".n-inner").animate({'opacity':'1'},500);
			});
			newsToSet = i+1;
		}
