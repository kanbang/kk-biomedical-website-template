...
if($isAdmin)
{
	...
	if($_GET["mode"] == 'reply')
	{
		...
		else
		{
			...
			if($row=mysql_fetch_row($query))
			{
				...
				{
					$body= '
					<div style="color:#999999;font-size:15px;direction:ltr;text-align:right;">
					Dear '.$name.'<br>
					Email: '.$email.'<br><br>
					<b>Your message:</b><br>
					'.$row[2].'<br><br>
				...
				if(!$mail->Send()) {
					...
					?>
					<textarea class="textbykk" style="width: 97.5%;height: 50px;" id="reply_index" onfocus="if(this.value == 'پاسخ...') this.value='';" id="reply_text<?php echo $rows[0]; ?>" >پاسخ...</textarea>
					<div style="width: 100%;padding-top: 3px;height: 18px;">
						<input type="submit" class="btnbykk" value="send" style="float: left;width: 100px;margin-left: 5px;" >
					</div>
					<?php
				}
				else 
				{
					?>
					<span style="float: right;color: #777;clear: right;">پاسخ شما:</span><br>
					<?php 
					$buff = explode("\n", $row[3]); 
	...
	?>
		...
		<div class="header" >
			<span class="mtitle" >نام:</span><span class="mdata" ><?php echo $mrow[2]; ?></span>
			<span class="mtitle" >ایمیل:</span><span class="mdata" ><?php echo $mrow[1]; ?></span>
			<?php if($mrow[3] != '') { ?><span class="mtitle" >تلفن همراه:</span><span class="mdata" ><?php echo $mrow[3]; ?></span><?php } ?>
			<?php if($mrow[4] != '0'){ ?><span class="mtitle"  onclick="msgMEdit(<?php echo $mrow[4]; ?>);" >لینک کاربر</span><?php } ?>
		</div>
		<div class="message">
		...
		?>
			<textarea class="textbykk" style="width: 97.5%;height: 50px;" id="reply_index" onfocus="if(this.value == 'پاسخ...') this.value='';" id="reply_text<?php echo $rows[0]; ?>" >پاسخ...</textarea>
			<div style="width: 100%;padding-top: 3px;height: 18px;">
				<input type="button" class="btnbykk" value="send" style="float: left;width: 100px;margin-left: 5px;" onclick="sendReply(<?php echo $mrow[0]; ?>);" >
			</div>
		<?php 
		}
		?>
	...
	?>
		...
		<div id="delete_dialogMsg" title="حذف پیام" style="display: none;">
		...
		<div style="opacity:0;width: 100%;height: 100%;" id="index-loader">
			<div id="messages-container">
				...
				$query = mysql_query("SELECT id, name, email, jdate, jtime, mlang FROM messagebykk WHERE state = 1 ORDER BY id  DESC ;", $db);
				if (!$query)
					die("Error reading query: ".mysql_error());
				...
				while($mrows = mysql_fetch_row($query))
				{
					...
					<div class="mitem <?php if($mrows[0] == $selected_id) echo "selected"; ?>" onclick="showMsg(<?php echo $mrows[0]; ?>);" >
						...
						<div class="text" ><a href="?command=<?php echo encodeURIComponent('page=message|&|mode=show_msg|&|section_id='.$mrows[0]); ?>" ><span style='font-weight: bold;' >نام: </span><?php echo $mrows[1]; ?><br><span style='font-weight: bold;' >ایمیل: </span><?php echo $mrows[2]; ?></a></div>
					</div>
					<?php 
				}
				?>
			...
			$(".mitem").mouseenter(function(){
				$(this).children(".text").stop().animate({'padding-right':'20px'},300);
			}).mouseleave(function(){
				if(!$(this).hasClass("selected"))
					$(this).children(".text").stop().animate({'padding-right':'10px'},300);
			}).click(function(){
				$(".mitem").removeClass("selected");
				$(".mitem").not(this).children(".text").animate({'padding-right':'10px'},300);
