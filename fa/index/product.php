...
if($_GET["show"] == '1')
{
	...
	<h1 id="product-title" ><?php echo $prow[1];?><br><span><?php echo $prow[2];?></span></h1>
		<div id="product-tabs" >
			...
			if($prow[20] != '' && (($prow[21] <= $level && $prow[21] > 0) || $isAdmin)){?><div class="tab" rel=".tab-5" ><span>دانلود ها</span></div><?php }
			?>
		...
		<div class="tab-container tab-5" >
			<div class="tab-inner">
				...
				foreach ($attachments as $attachment)
				{
					...
					{
				?>
				<div class="news-attachment" ><img src="../images/attachment.png" ><a href="product/attachment/<?php echo $attachment;?>" class="download" target="_blank">دانلود(<?php echo $attachment; ?>)</a></div><br>
				<?php
					}
				}
				?>
	...
	$("#controlpanel_container .controls").empty().css({'opacity':'0.0'}).append(
		"<div class='btn add' onclick='addProduct();' title='افزودن محصول' ></div><div class='btn change' onclick='editProduct(<?php echo $prow[0]; ?>);' title='تغییر محصول' ></div><div class='btn delete' onclick='deleteProduct(<?php echo $prow[0]; ?>);' title='حذف محصول' ></div>"
	...
	$("#product-tabs .tab").mouseenter(function(){
		$("#product-tabs .tab").not(this).stop().animate({'margin-right':'0','padding-right':'25px'},200);
		$(this).stop().animate({'margin-right':-($(this).children("span").width())+'px','padding-right':'10px'},200);
	}).mouseleave(function(){
		$("#product-tabs .tab.selected").stop().animate({'margin-right':-($("#product-tabs .tab.selected span").width())+'px','padding-right':'10px'},200);
		$("#product-tabs .tab").not(".selected").stop().animate({'margin-right':'0','padding-right':'25px'},200);
	});
	...
	$("#close-product").click(function(){
		if($("#product-index").hasClass("open"))
		{
			...
			$("#controlpanel_container .controls").empty().css({'opacity':'0.0'}).append(
				"<div class='btn add' onclick='addProduct();' title='افزودن محصول' ></div>"
				).animate({'opacity':'1'},300);
	...
	$("#product-index .primary .secondary .container .tab-container").mouseenter(function(){
		$("#product-tabs .tab").stop().animate({'margin-right':'0','padding-right':'25px'},200);
	}).mouseleave(function(){
		$("#product-tabs .tab.selected").stop().animate({'margin-right':-($("#product-tabs .tab.selected span").width())+'px','padding-right':'10px'},200);
		$("#product-tabs .tab").not(".selected").stop().animate({'margin-right':'0','padding-right':'25px'},200);
	});
	...
	setTimeout(function(){
		$("#product-tabs .tab.selected").css({'margin-right':-($("#product-tabs .tab.selected span").width())+'px','padding-right':'10px'});
		$("#product-tabs .tab").not(".selected").css({'margin-right':'0','padding-right':'25px'});
...
<?php 
	...
	{
	?>
	<div id="delete_dialogP" title="حدف محصول" style="display: none;">
...
<div style="opacity:0;width: 100%;height: 100%;" id="index-loader">
			...
			if($check2 == 0)
			{
				...
				<div style="background-image: url('<?php echo $allrows[$start_counter+$i][4]; ?>');" class="item small" >
					...
					<div class="control-container">
						...
						<div class="visit-counter" >بازدید: <?php echo $allrows[$start_counter+$i][7]; ?></div>
					</div>
					<?php 
					}
					?>
			...
			else if($check2 == 1)
			{
				...
				for($i=0;$i<5;$i++)
				{
					...
					if($allrows[$start_counter+$i][5] == 1)
					{
						$check1 = true;
					?>
					<div style="background-image: url('<?php echo $allrows[$start_counter+$i][3]; ?>');<?php if(!$parity) echo "float:left;"?>" class="item big"  >
						<?php 
						if($isAdmin)
						{
						?>
					...
					{
					?>
					<div style="background-image: url('<?php echo $allrows[$start_counter+$i][4]; ?>');<?php if(!$parity) echo "float:left;"?>" class="item small" >
						<?php 
						if($isAdmin)
						{
						?>
	...
	$(document).ready(function(){
	    ...
	    $("#product-items .item a").mouseenter(function(){
	    	...
	    	$(this).children(".p-inner").children(".first").css({'width':pFirstDetailW+'px','left':(-pFirstDetailOW)+'px','display':'block'});
	    	$(this).children(".p-inner").children(".second").css({'width':pSecondDetailW+'px','right':(-pSecondDetailOW)+'px','display':'block'});
	    	
		    $(this).children(".p-inner").removeClass("none-pattern").stop().animate({'opacity':'1'},300);
		    $(this).children(".p-inner").children(".first").stop().animate({'left':'0px'},300);
		    $(this).children(".p-inner").children(".second").stop().animate({'right':'0px'},300);
