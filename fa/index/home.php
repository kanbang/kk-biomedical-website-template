	...
	<div id="home-container">
		<div id="most-visited-product" >
			<h1 class="home-title" style="line-height: 1em;" >
				محصولات<br><span style="font-size: 10px;line-height: 1em;"> (بیشترین بازدید)</span>
			</h1>
		...
		<div id="most-recent-product" >
			<h1 class="home-title" style="line-height: 1em;" >
				محصولات<br><span style="font-size: 10px;line-height: 1em;"> (جدیدترین)</span>
			</h1>
		...
		<div id="most-recent-news">
			<h1 class="recent-news-title" style="line-height: 1em;" >
				اخبار<br><span style="font-size: 10px;line-height: 1em;"> (جدیدترین)</span>
			</h1>
			<div class="mrn-scroll">
				<div class="mrn-container">
					<?php 
				    if($isAdmin)
				    	$cquery = mysql_query("SELECT id, name, summary, picture_thumb, jdate, jtime FROM newsbykk_".$sitelang." WHERE state = 1 ORDER BY id DESC ;", $db);
				    else
				    	$cquery = mysql_query("SELECT id, name, summary, picture_thumb, jdate, jtime FROM newsbykk_".$sitelang." WHERE state = 1 AND showto <= '$level' AND showto > 0  ORDER BY id DESC ;", $db);
				    if (!$cquery)
				    	die("Error reading query: ".mysql_error());
...
$("#most-visited-product").mouseenter(function(){
	...
	$("#mvp-container").stop().animate({'margin-right':'0px'},300);
	...
	$("#mvp-container").stop().animate({'margin-right':'-25px'},300);
...
$(document).ready(function(){
	window.document.title = window.document.title= "<?php echo $sitetitle." | "; ?>صفحه اصلی";
