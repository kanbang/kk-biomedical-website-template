...
if($isAdmin)
{
		...
		while($mrows = mysql_fetch_row($query))
		{
			...
			<div class="field_tr <?php if($mrows[10] == 2) echo "red";?>" id="mf<?php echo $mrows[0];?>" >
				...
				<div class="member_field <?php if($i%2 == 1) echo "fodd"; else echo "feven";?>" style="width: 75px;" <?php if($allowEit) { echo'onclick="mEdit('.$mrows[0].');"'; }?>>
					<?php 
					if($mrows[6] == '2') echo "User";
					if($mrows[6] == '3') echo "Level 1";
					if($mrows[6] == '4') echo "Level 2";
					if($mrows[6] == '5') echo "Level 3";
					if($mrows[6] == '7') echo "Admin";
					if($mrows[6] == '10') echo "F.A.A.";
					?>
				</div>
				...
				<div class="member_field <?php if($i%2 == 1) echo "fodd"; else echo "feven";?>" style="cursor: default;padding-top: 0;height: 23px;">
					<div style="height: 20px;width: 90px;padding: 3px 10px 0 0;margin: 0px;float: left;">
						<?php 
						if($allowEit)
						{
						?>
						<img src="../images/wrongbw.png" title="Delete" onmouseover="this.src='../images/wrong.png';" onmouseout="this.src='../images/wrongbw.png';" style="float: left;margin: 0 0 0 10px;height: 20px;cursor: pointer;" onclick="delMember(<?php echo $mrows[0];?>);">
						<?php 
						}
						if($mrows[9] != 1)
						{
						?>
						<img src="../images/correctbw.png" title="Confirm" onmouseover="this.src='../images/correct.png';" onmouseout="this.src='../images/correctbw.png';" style="float: left;margin: 0 0 0 10px;height: 20px;cursor: pointer;" onclick="confirmMember(this,<?php echo $mrows[0];?>);">
						<?php 
						}
						?>
					</div>
	...
	else if($_GET["mode"] == 'delete')
	{
		...
		else if($check && $askForActiovation)
		{
			...
			if($row = mysql_fetch_row($query))
			{
				...
				با سلام'.$firstname.' '.$lastname.'<br>
				ایمیل: '.$email.'<br>
				حساب کاربری شما تایید نشد!<br>
				با تشکر
				</div>';
				...
				$mail->Subject    = "عدم تایید حساب کاربری";
	...
	else if($_GET["mode"] == 'confirm' && $askForActiovation)
	{
		...
		else
		{
			...
			if($row = mysql_fetch_row($query))
			{
				...
				با سلام'.$firstname.' '.$lastname.'<br>
				ایمیل: '.$email.'<br>
				حساب کاربری شما تایید شد!<br><span style="color:red;font-weight:bold;" >
				کد کاربری: '.$row[0].'</span><br>
				با تشکر
				</div>';
				...
				$mail->Subject    = "ØªØ§ÛŒÛŒØ¯ Ø­Ø³Ø§Ø¨ Ú©Ø§Ø±Ø¨Ø±ÛŒ";
	...
	else if($_GET["mode"] == 'edit' || $_GET["mode"] == 'applyedit')
	{
		...
		if($mrow=mysql_fetch_row($query))
		{
			...
			<div class="div-title">EDIT USER</div>
			<form action="index/member.php?mode=applyedit" method="POST" id="edituser_form" >
				...
				<div class="div-row" >
					...
					<div class="input-div" style="position: relative;">
						...
						<div style="float: left;padding: 7px 5px 5px 0;font-size: 13px;position: absolute;bottom: 10px;left:33px;">+</div><input type="text"  style="width: 385px;padding-left: 20px;" class="textbykk" id="ucellphone" name="ucellphone" value="<?php echo $mrow[4]; ?>" disabled="disabled" >
					</div>
				</div>
				<div class="div-row" >
					<div class="input-div" >
					...
					<div class="input-div" >
						<div class="lable" ><span class="ued" >*</span>User level</div>
						<select class="textbykk" name="user_level" id="user_level" style="width: 412px;" >
							<option value="2" <?php if($mrow[9] == 2) echo "selected";?> >Normal user</option>
							<option value="3" <?php if($mrow[9] == 3) echo "selected";?> >Level 1</option>
							<option value="4" <?php if($mrow[9] == 4) echo "selected";?> >Level 2</option>
							<option value="5" <?php if($mrow[9] == 5) echo "selected";?> >Level 3</option>
							<option value="7" <?php if($mrow[9] == 7) echo "selected";?> >Admin</option>
							<?php if($_SESSION["adminbykk"][0] == '99999') { ?><option value="10" <?php if($mrow[9] == 10) echo "selected";?> >Full Access Admin (F.A.A.)</option><?php } ?>
						</select>
					</div>
				</div>
				<div class="div-row" >
				...
				<div class="div-row" >
					<div style="padding: 20px 25px 5px;float: left" id="uchange-pass">
							<input type="button" value="Change Password" style="width: 850px;" class="btnbykk" >
					</div>
					<div id="uchange-pass-container" style="display: none;" >
						<div class="input-div">
							<div class="lable">News Password <span style="color: red;font-size: 11px;display: none;float: right;" id="upass6minalert"> 6 characters min!</span></div>
				...
				<div class="div-row" >
					<div class="input-div" >
						<input type="submit" class="btnbykk" value="Apply / Unlock" style="width: 412px;">
					</div>
				</div>
			</form>
			<script>
	...
	<div id="delete_dialogM" title="Delete Member" style="display: none;">
	...
	<div style="opacity:0;width: 100%;height: 100%;" id="index-loader">
		<div id="member-container">
			<div class="mheader">
				...
				<input type="text" id="msearch" style="float: left;width: 150px;font-size: 10px;" class="textbykk">
				<span style="float: right;margin: 0;padding: 2px;">
					<a href="index/exportexcel.php?i1=1&i2=1&i3=1&i4=1&im=1" target="_blank" id="export_excel" class="download" ><img alt="excel export" src="../images/excelexportbw.png" title="Ø®Ø±ÙˆØ¬ÛŒ excel 2007 .xlsx" onmouseover="this.src='../images/excelexport.png'" onmouseout="this.src='../images/excelexportbw.png';"  style="margin-left: 10px;height: 25px;width: 25px;" /></a>
				</span>
				<span style="float: right;margin: 0;padding: 3px;">
					<input type="checkbox" id="inc_1" class="inc" checked="checked">Users
					<input type="checkbox" id="inc_2" class="inc" checked="checked">Level 1
					<input type="checkbox" id="inc_3" class="inc" checked="checked">Level 2
					<input type="checkbox" id="inc_4" class="inc" checked="checked">Level 3
					<input type="checkbox" id="inc_m" class="inc" checked="checked">Admin
				</span>
			</div>
