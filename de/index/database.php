			...
			if($lrow[2] == $lpassword)
			{
				...
				if($lrow[8] == 1)
				{
					if($lrow[7] == 1)
					{
						...
						$_SESSION["login_msg"][1] = "Willkommen ".$lrow[3]."!";
				...
				else if($lrow[8] == 2)
				{
					...
					$_SESSION["login_msg"][1] = "Ihr Konto wird nicht akzeptiert noch!!!";
				}
			}
			else
			{
				...
				$_SESSION["login_msg"][1] = "Falsches Passwort! Für Passwort-Wiederherstellung folgen <a href='index/activeusermail.php?mode=recoverpass&email=".$lrow[1]."' target='_blank' style='color:#aaa;' class='download' >Diese</a> Link";
			}
		}
		else
		{
			$_SESSION["login_msg"][0] = "0";
			$_SESSION["login_msg"][1] = "E-Mail oder Passwort ist falsch!";
		}
	}
	else
	{
...
if(isset($_GET["logout"]) && $_GET["logout"] == '1' && (isset($_SESSION["adminbykk"]) || isset($_SESSION["userbykk"])) )
{
	...
	{
		$_SESSION["login_msg"][0] = "1";
		$_SESSION["login_msg"][1] = "Sie haben sich erfolgreich abgemeldet!";
	}
}
$isAdmin = false;
if(isset($_SESSION["adminbykk"]))
